...
{
    public GameObject ImageD; // Asigna la ImageA de la letra A desde el Inspector
    ...
    private void Start()
    {
        ImageD.SetActive(false); // Asegúrate de que la ImageA esté oculta al inicio
        if (key != null)
        {
    ...
    public void SoltarLlave()
    {
        if (!cofreAbierto)
        {
            ...
            // Mostrar la imagen
            ImageD.SetActive(true); // Aparece la Imagen A
